...
-- Key Input Tab
local KeyTextbox
KeyTab = Tabs.KeyInput
KeyTab:AddTextbox({
    Title = "Enter Key",
    Description = "Type your key here",
    Placeholder = "Type here...",
    Callback = function(input)
        -- Submit handled by button
    end
})
KeyTextbox = KeyTab:GetChildren()[1] -- assumes first child is textbox

KeyTab:AddButton({
    Title = "Submit Key",
    Description = "Submit the key you typed",
    Callback = function()
        local input = KeyTextbox.Value or ""
        if input == REQUIRED_KEY then
            Fluent:Notify({ Title = "MASTXR Hub", Content = "Loading menu...", Duration = 3 })
            wait(3)
            keyUnlocked = true
            ...
            Tabs.KeyInput:SetDisabled(true)
            Window:SelectTab(2) -- Switch to first real tab
        else
            Fluent:Notify({ Title = "MASTXR Hub", Content = "Invalid Key!", Duration = 3 })
        end
    end
})
