...
-- Optional: initial notification at script execute
...
local KEY_SEQUENCE = {Enum.KeyCode.K, Enum.KeyCode.E, Enum.KeyCode.Y}
local sequenceIndex = 1

UIS.InputBegan:Connect(function(input, gameProcessed)
    if gameProcessed then return end
    if input.UserInputType == Enum.UserInputType.Keyboard then
        if input.KeyCode == KEY_SEQUENCE[sequenceIndex] then
            sequenceIndex = sequenceIndex + 1
            if sequenceIndex > #KEY_SEQUENCE then
                -- Sequence complete
                sequenceIndex = 1
