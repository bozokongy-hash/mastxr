...
-- Create a temporary Fluent window for key input
local KeyWindow = Fluent:CreateWindow({
    Title = "MASTXR Hub - Key Required",
...
-- Single tab for key input
local KeyTab = KeyWindow:AddTab({ Title = "Key Input", Icon = "key" })
...
-- Optional Submit Button (calls textbox callback)
KeyTab:AddButton({
    Title = "Submit Key",
    Description = "Submit the key you typed",
    Callback = function()
        local textbox = KeyTab:GetChildren()[1] -- assumes first child is textbox
        local input = textbox.Value or ""
        if input == REQUIRED_KEY then
            Fluent:Notify({
                Title = "MASTXR Hub",
                Content = "Loading menu...",
